...
import { requireAuth, requireCompleteProfile } from "../../middlewares/auth.middleware.js";

const router = Router();

router.get("/", requireAuth, controllers.getBookings);
router.get("/:id", requireAuth, controllers.getBookingById);
router.post("/", requireAuth, requireCompleteProfile, validateRequest(createBookingSchema), controllers.createBooking);
