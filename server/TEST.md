...
### D. Upload Hostel Images
- **Method**: `POST`
- **URL**: `{{base_url}}/api/hostels/{{hostel_id}}/images`
- **Body**: `form-data`
  - Key: `images` (type: File) - Attach up to 10 JPEG/PNG files.
...
### E. Upload Room Images
- **Method**: `POST`
- **URL**: `{{base_url}}/api/rooms/{{room_id}}/images`
- **Body**: `form-data`
  - Key: `images` (type: File) - Attach up to 10 JPEG/PNG files.

### F. Add Room Tour Scene
- **Method**: `POST`
- **URL**: `{{base_url}}/api/rooms/{{room_id}}/tours`
- **Body** (Raw JSON):
```json
{
  "scene_name": "Main Bedroom",
  "scene_config_url": "https://your-supabase-url.supabase.co/storage/v1/object/public/panoramas/unique_folder/config.json"
}
```
*(Used to link multiple panoramas to a single room. Send the config.json URL generated by the Python Pannellum Microservice)*
---
